...
class ListIp extends RiskCommand
{
    ...
    public function handle($message, $match = []) { if (!$this->authorized($message)) return; $this->sendIndicatorPage('ip', 'IP 风险名单', 'risk_ip', (int)($message->args[0] ?? 1), (int)$message->chat_id, 0, (int)($message->message_id ?? 0)); }
